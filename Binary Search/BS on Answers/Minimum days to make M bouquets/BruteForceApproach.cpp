...
int mini_days_toMake_bouquets_BR(vector<int> &a, int n, int m, int k)
{
  // find maximum and minimum:
  int mini = mini_days(a, n);
  int maxi = max_el_for_koko(a, n); // this will give the max. element in the arr..
  if (n < m * k)
    return -1; // edge case..
  for (int i = mini; i <= maxi; i++)
  {
    if (possible_toMake_bouquets(a, n, i, m, k) == 1)
    {
      return i;
    }
  }
...
int main(){
  ...
  cout<<mini_days_toMake_bouquets_BR(a,n,m,k);
