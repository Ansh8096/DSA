...
vector<int> findNGE(vector<int>&a,int n){
    vector<int> NGE(n);
    stack<int>stk;
    for (int i = n-1; i >= 0; i--)
    {
...
vector<int> findPGEE(vector<int>&a,int n){
    vector<int> PGEE(n);
    stack<int>stk;
    for (int i = 0; i < n; i++){
        while (!stk.empty() and a[stk.top()] < a[i]) stk.pop();
...
int sumOfMaxOfSubArr_OP(vector<int>& a,int n){
    vector<int> NGE = findNGE(a,n);
    vector<int> PGEE = findPGEE(a,n);
...
int SumOfSubmissionOfRange_OP(vector<int> a,int n){
    return sumOfMaxOfSubArr_OP(a,n) - sumOfMinOfSubArr_OP(a,n);
}
int main(){
    ...
    cout<<SumOfSubmissionOfRange_OP(a,n);
